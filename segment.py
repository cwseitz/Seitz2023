    ...
    def segment(self):
        ...
        for n in range(nt):
            ...
            viewer.add_shapes(contours, shape_type='polygon', edge_width=5,
                          edge_color='red', face_color='white', opacity=0.3, name='UNET Prediction')
            napari.run()
...
"""
class Summary:
    def __init__(self,datapath,analpath,prefix,cell_filters,nucleus_filters,p0=0.95,z0=4):
        self.datapath = datapath
        self.analpath = analpath
        self.prefix = prefix
        self.z0 = z0
    ...
    def get_mask(self,sfmx):
        ...
        nmask = img_as_bool(nmask)
        return nmask 
    def separate_objects(self,image):
    ...
    def summarize(self):
        nz,nc,nt,nx,ny = self.rawdata.shape
        for n in range(50,nt):
            ch0 = resize(np.max(self.rawdata[:,0,n,:,:],axis=0),(256,256))
            ch1 = resize(np.max(self.rawdata[:,1,n,:,:],axis=0),(256,256))
            ch0_mask = self.get_mask(self.ch0_sfmx[n])
            ch1_mask = self.get_mask(self.ch1_sfmx[n])
            ch0_mask_sep = self.separate_objects(ch0_mask)
            ch1_mask_sep = self.separate_objects(ch1_mask)
            #self.plot_prob(self.ch0_sfmx,ch0,ch0_mask,ch0_mask_sep)
            #self.plot_prob(self.ch1_sfmx,ch1,ch1_mask,ch1_mask_sep)
            ...
            rgb = self.get_rgb(np.max(self.rawdata[:,0,n,:,:],axis=0),
                               np.max(self.rawdata[:,1,n,:,:],axis=0),
                               np.max(self.rawdata[:,2,n,:,:],axis=0))
            #rgb = mark_boundaries(rgb,ch0_mask,mode='thick',color=(1,1,1))
