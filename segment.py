...
class SpotCounts:
    ...
    def summarize(self):
        ...
        for n in range(nt):
            ...
            ch1_spotst, ch2_spotst = self.assign_spots_to_cells(ch1_spotst,ch2_spotst,mask)
            ...
            rgb = mark_boundaries(rgb,mask,mode='thick',color=(1,1,1))
            ax.imshow(rgb)
            anno_blob(ax,ch1_spotst,color='cyan')
            anno_blob(ax,ch2_spotst,color='yellow')
            ax.set_xticks([]); ax.set_yticks([])
            plt.tight_layout()
            plt.savefig(self.analpath+self.prefix+'/'+self.prefix+f'_summary{n}.png')
            plt.show()
